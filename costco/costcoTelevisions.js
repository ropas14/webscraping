...
var fs = require('fs');
const start_url = "https://www.costco.com/televisions.html?sortBy=PriceMin%7C0";
let pagesVisited = {};
let promises = [];
let pagesUrls = [];
...
async function collectLinks($,url) {
  var links = $('div.c_282623 p.description a');
  var pagelinks = $('div.c_282623  div.paging li.page a');
     if (links) {
      links.each(function() {
...
fs.writeFile("costcoTVs.json", jsonContent, 'utf8', function(err) {
   if (err) {
