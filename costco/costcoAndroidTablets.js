...
var fs = require('fs');
const start_url = "https://www.costco.com/CatalogSearch?dept=All&keyword=android+tablets";
let pagesVisited = {};
let promises = [];
let pagesUrls = [];
...
async function collectLinks($,url) {
  var links = $('div#search-results p.description a');
     if (links) {
      links.each(function() {
...
fs.writeFile("costcoAndroidTablets.json", jsonContent, 'utf8', function(err) {
   if (err) {
